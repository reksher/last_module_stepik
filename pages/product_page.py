...
class PromoPage(BasePage):
    ...
    def should_be_correct_notification(self):
        assert 'был добавлен в вашу корзину.' in self.browser.find_element(*PromoPageLocators.NOTIFICATION_OF_ADD).text, f'Notification of adding to the cart is incorrect, actual result {self.browser.find_element(*PromoPageLocators.NOTIFICATION_OF_ADD).text}'
    ...
    def should_be_price_notification(self):
        assert 'Стоимость корзины теперь составляет' in self.browser.find_element(*PromoPageLocators.NOTIFICATION_PRICE).text, (f'Notification indicating the cost is incorrect, actual result {self.browser.find_element(*PromoPageLocators.NOTIFICATION_PRICE).text}')
        # Проверка, что текст нотификации корректен

    def should_be_correct_notification_for_price(self):
        assert self.browser.find_element(*PromoPageLocators.PRICE_BOOK).text in self.browser.find_element(*PromoPageLocators.NOTIFICATION_PRICE_IN_BASKET).text, f'The price in the notification is incorrect, actual result {self.browser.find_element(*PromoPageLocators.NOTIFICATION_PRICE_IN_BASKET).text}'
